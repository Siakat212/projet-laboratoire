...
import { Calendar, ArrowLeft, Trophy, Users, BookOpen, Image, X } from "lucide-react";
  ...
  const getCategoryColor = (category: string) => {
    ...
    <section className="py-16 bg-grid" style={Constants.pdTopCardApresNav}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <Link to="/news" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 mb-8">
                  ...
                  <div className="flex items-center gap-3 mb-2 flex-wrap justify-between">
                    <div className="flex items-center gap-2 flex-wrap">
                      {newsItem.typeNew.map((type, id) => (
                        <Badge key={id} className={getCategoryColor(type.typeNew)}>
                            {type.typeNew}
                        </Badge>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-muted-foreground text-sm">
                        ...
                        {newsItem.date_realisation_fin ? dateFormatter(newsItem.date_realisation_debut) + ' - ' + dateFormatter(newsItem.date_realisation_fin) : dateFormatter(newsItem.date_realisation_debut)}
                    </div>
                  </div>
            ...
            {newsItem.image_principal && (
              <div className="px-6 pb-6">
                ...
                <div className="mb-6">
                  ...
                  >
                    <img
                      src={`${Constants.url}${newsItem.image_principal}`}
                      alt={newsItem.titre}
                  ...
                  >
                    {newsItem.contenu_complet.split('\n').map((line, index) => {
                      if (line.startsWith('<h3>') && line.endsWith('</h3>')) {
                        const headingText = line.replace('<h3>', '').replace('</h3>', '');
                        return <h3 key={index} className="text-2xl font-semibold mt-8">{headingText}</h3>;
                      } else if (line.startsWith('<p>') && line.endsWith('</p>')) {
                        const paragraphText = line.replace('<p>', '').replace('</p>', '');
                        return <p key={index} className="mt-4">{paragraphText}</p>;
                      } else {
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden p-0">
            {selectedImage && (
              <div className="relative">
                <img
                  src={`${Constants.url}${selectedImage}`}
                  alt='image'
