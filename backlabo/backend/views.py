...
def enteteGeneral(request, id_laboratoire):
    data = LaboratoirePageBlockEntete.objects.filter(id_laboratoire__id = id_laboratoire)
    return Response({'data': LaboratoirePageBlockEnteteSerializers(data, many=True).data})


@api_view(['GET'])
def presentation(request, id_laboratoire):
    data = LaboratoirePresentation.objects.filter(id_laboratoire__id = id_laboratoire)
    return Response({'data': LaboratoirePresentationSerializers(data, many=True).data})

@api_view(['GET'])
def presentationImage(request, id_laboratoire):
    data = LaboratoirePresentationImage.objects.filter(id_laboratoire__id = id_laboratoire).last()
    return Response({'data': LaboratoirePresentationImageSerializers(data, many=False).data})

@api_view(['GET'])
def presentationMission(request, id_laboratoire):
    data = LaboratoireMission.objects.filter(id_laboratoire__id = id_laboratoire).last()
    return Response({'data': LaboratoireMissionSerializers(data, many=False).data})

@api_view(['GET'])
def presentationDomaine(request, id_laboratoire):
    data = LaboratoireDomaine.objects.filter(id_laboratoire__id = id_laboratoire)
    return Response({'data':LaboratoireDomaineSerializers(data, many=True).data})

@api_view(['GET'])
def presentationSlider(request, id_laboratoire):
    data = LaboratoireSlider.objects.filter(id_page__titre = 'about', id_laboratoire__id = id_laboratoire).last()
    return Response({'data':PresentationSliderSerializers(data, many=False).data})

@api_view(['GET'])
def presentationPartenaire(request, id_laboratoire):
    data = LaboratoirePartenaire.objects.filter(id_laboratoire__id = id_laboratoire, statu = "Actif")
    return Response({'data':PresentationPartenaireSerializers(data, many=True).data})

@api_view(['GET'])
def acceuilSlider(request, id_laboratoire):
    data = LaboratoireSlider.objects.filter(id_page__titre = 'acceuil', id_laboratoire__id = id_laboratoire).last()
    return Response({'data': AcceuilSliderSerializers(data, many=False).data})

@api_view(['GET'])
def acceuilPresentation(request, id_laboratoire):
    data = LaboratoireSlider.objects.filter(id_page__titre = 'acceuil', id_laboratoire__id = id_laboratoire).last()
    return Response({'data': AcceuilPresentationSerializers(data, many=False).data})

@api_view(['GET'])
def acceuilDirecteur(request, id_laboratoire):
    data = ChercheurLaboratoire.objects.filter(id_laboratoire__id = id_laboratoire, statu = "Actif", id_chercheur_poste__id_poste__nom = "Directeur du Laboratoire").last()
    return Response({'data': acceuilDirecteurSerializers(data, many=False).data})

@api_view(['GET'])
def acceuilChiffre(request, id_laboratoire):
    data = Laboratoire.objects.get(id = id_laboratoire)
    return Response({'data': acceuilChiffreCleSerializers(data, many=False).data})

@api_view(['GET'])
def acceuilActualite(request, id_laboratoire):
    data = LaboratoireNew.objects.filter(id_laboratoire__id = id_laboratoire).order_by('-creer_le')[:2]
    return Response({'data': ActualiteSerializers(data, many=True).data})


@api_view(['GET'])
def actualiteDetail(request, id_laboratoire , id):
    data = LaboratoireNew.objects.get(id_laboratoire__id = id_laboratoire, id= id)
    return Response({'data': ActualiteSerializers(data, many=False).data})


@api_view(['GET'])
def actualiteSlider(request, id_laboratoire):
    data = LaboratoireSlider.objects.filter(id_page__titre = 'new', id_laboratoire__id = id_laboratoire).last()
    return Response({'data':ActualiteSliderSerializers(data, many=False).data})


@api_view(['GET'])
def actualiteAll(request, id_laboratoire):
    data = LaboratoireNew.objects.filter(id_laboratoire__id = id_laboratoire).order_by('-creer_le')
    return Response({'data':ActualiteSerializers(data, many=True).data})


@api_view(['GET'])
def rechercheSlider(request, id_laboratoire):
    data = LaboratoireSlider.objects.filter(id_page__titre = 'research', id_laboratoire__id = id_laboratoire).last()
    return Response({'data': RechercheSliderSerializers(data, many=False).data})


@api_view(['GET'])
def rechercheAll(request, id_laboratoire):
    data = RechercheLaboratoire.objects.filter(id_laboratoire_domaine__id_laboratoire__id = id_laboratoire).order_by('-creer_le')
    return Response({'data': RechercheLaboratoireSerializers(data, many=True).data})


@api_view(['GET'])
def detailRecherche(request, id_laboratoire, id):
    data = RechercheLaboratoire.objects.get(id_laboratoire_domaine__id_laboratoire__id = id_laboratoire, id = id)
    return Response({'data': RechercheLaboratoireDetailSerializers(data, many=False).data})


@api_view(['GET'])
def directeur(request, id_laboratoire):
    data = ChercheurLaboratoire.objects.get(id_laboratoire = id_laboratoire, id_chercheur_poste__id_poste__nom = "Directeur du Laboratoire")
    return Response({'data': ChercheurLaboratoireSerializers(data, many=False).data})


@api_view(['GET'])
def directeurMotDirecteur(request, id_laboratoire, id):
    data = ChercheurMot.objects.get(id= id)
    return Response({'data': ChercheurMotSerializers(data, many=False).data})
...
class RecherchePublicationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filtre par id de recherche
        recherchePublication_id = self.request.query_params.get('id')
        if recherchePublication_id and recherchePublication_id.isdigit():
            qs = qs.filter(id=int(recherchePublication_id))
            print(qs)

        # Filtre par laboratoire (par défaut: laboratoire ID 1)
        laboratoire_id = self.request.query_params.get('laboratoire_id', '0')
        try:
            lab_id_int = int(laboratoire_id)
        except ValueError:
            lab_id_int = 0  # Valeur par défaut si conversion échoue
